...
#define I_GAIN          (0.25)   // Integral gain
#define D_GAIN          (0.15)   // Derivative gain
...
#define COMMAND         (15)     // Commanded/Requested pitch (in degrees from horizontal)
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    // Otherwise, treat input as new pitch request
      ...
      bool zeroCommanded = Serial.peek() == '0';
      float newAngle = Serial.parseFloat();
      if(newAngle == 0.00 && !zeroCommanded){
        newAngle = controller.input;
      }
